...
public class PropositionInterpreter {
    ...
    public PropositionInterpreter(String proposition, boolean simple) {
        ...
        if (simple) {
            ...
            HashMap<String, Boolean> truthmaps = complex_prop_hashmap(parse.parsed);
    ...
    public static HashMap<String, Boolean> complex_prop_hashmap(Expr root) {
        ...
        HashMap<String, Boolean> truthmaps = new HashMap<>();

        // Call on recursive helper method to update truthmaps
        // Initialize all values to false
        complex_prop_hashmap_helper(root, truthmaps);
    ...
    private static void complex_prop_hashmap_helper(Expr root, HashMap<String, Boolean> truthmaps) {
        // Traverse the tree left and right,
        if (root instanceof Expr.BinaryOperation) {
            // Go left and right
            complex_prop_hashmap_helper( ((Expr.BinaryOperation)root).left, truthmaps );
            complex_prop_hashmap_helper( ((Expr.BinaryOperation)root).right, truthmaps );
        } else if (root instanceof Expr.Unary) {
            // UNARY Operations, go into expression
            complex_prop_hashmap_helper( ((Expr.Unary)root).expression, truthmaps );
        } else if (root instanceof Expr.Grouping) {
            // Groupings
            complex_prop_hashmap_helper( ((Expr.Grouping)root).expression, truthmaps );
        } else {
            // Literals, just use truth values of the actual node
            truthmaps.putIfAbsent(((Expr.Literal) root).name.toString(), false);
        }
    }
    ...
    public static boolean complex_evaluator(Expr root, HashMap<String, Boolean> truthmaps) {
        // Evaluate according to an in-order traversal
        // Evaluates based on NAME tokens' truth values
        if (root instanceof Expr.BinaryOperation) {
            // Either an AND, OR, IMPLIES, or IFF
            switch(((Expr.BinaryOperation)root).operation.type) {
                case AND -> {
                    boolean left = complex_evaluator(((Expr.BinaryOperation)root).left, truthmaps );
                    boolean right = complex_evaluator(((Expr.BinaryOperation)root).right, truthmaps );
                    return Propositions.and(left, right);
                }
                case OR -> {
                    boolean left = complex_evaluator(((Expr.BinaryOperation)root).left, truthmaps );
            ...
            // UNARY Operations
            return Propositions.not( complex_evaluator( ((Expr.Unary)root).expression, truthmaps ) );
        } else if (root instanceof Expr.Grouping) {
            // Groupings
            return complex_evaluator( ((Expr.Grouping)root).expression, truthmaps );
        } else {
            // Literals, just use truth values of the actual node
            //return ((Literal) root).name.truth_value;
            if (truthmaps.get( ((Expr.Literal) root).name.toString() ) != null) {
                return truthmaps.get( ((Expr.Literal) root).name.toString() );
            } else {
